"""
Ported from beancount/parser/printer_test.py to be able to test the functionality of the printer_rich.py module
"""
...
from evbeantools.printer_rich import print_entries_rich
from evbeantools import printer_rich
...
def clean_empty_lines(text: str) -> str:
    """Removes empty lines from the text and right strips each line.
...
class TestEntryPrinter(cmptest.TestCase):
    """
    The sam as beanount.parser.printer_test.TestEntryPrinter, but tests, that the print_entries_rich
    returns the HTML -formatted text, which can be correctly parsed back to the entries, once a plain text is extracted 
    from it.

    Args:
        cmptest (_type_): _description_
    """
    def assertRoundTrip(self, entries1, errors1):
        ...
        print_entries_rich(entries2, file=oss2)
        
        oss2_html = oss2.getvalue()
        
        print('------------- oss2_html --------------')
        print(oss2_html)
        print('---------------------------------------')
        
        # oss2_html = oss2_html.replace('option "plugin_processing_mode" "raw"', '')
        
        oss2_txt = BeautifulSoup(oss2_html, features="html.parser").get_text()
        
        oss2_txt_with_plugin = 'option "plugin_processing_mode" "raw"\n' + oss2_txt
        
        # Remove empty lines and right strip each line, beacuse the BeautifulSoup.get_text() method
        # still leaves some empty lines from the original HTML code.
        oss2_txt_with_plugin_cleaned = clean_empty_lines(oss2_txt_with_plugin)
        
        print('------------- oss2_txt_with_plugin_cleaned --------------')
        print(oss2_txt_with_plugin_cleaned)
        print('--------------------------------------------------------')
        
        entries3, errors, __ = loader.load_string(oss2_txt_with_plugin_cleaned)

        self.assertEqualEntries(entries1, entries3)
        self.assertFalse(errors)
        
        oss1_value = oss1.getvalue()
        # Since we removed empty lines from the text, extracted from the HTML code, we need to do the same here.
        # to make the comparison correct.
        oss1_value = clean_empty_lines(oss1_value)

        print('------------- oss2_txt_with_plugin --------------')
        print(oss2_txt_with_plugin)
        print('------------------- oss1.getvalue() --------------------')
        print(oss1_value)

        # Compare the two output texts.
        self.assertEqual(oss2_txt_with_plugin_cleaned, oss1_value)
    ...
    def assertRoundTripViaRealFile(self, entries1, errors1, test_write_source=True):
        ...
        Args:
            ...
            every transaction
        """
      # TODO: remove this test when the issue is fixed
        pass
    ...
    @loader.load_doc()
    def test_TranSimp(self, entries, errors, __):
        """
        2014-01-01 open Assets:Account1
        2014-01-01 open Assets:Cash
        
        2014-06-08 * "Buying BEAN at cost"
          Assets:Account1       100.00 BEAN {50.00 USD} @ 50.00 USD
...
class MiscTests(test_utils.TestCase):
    """_summary_

    Args:
        test_utils (_type_): _description_
    """
    def test_amount_to_rich_string(self):
        amount1 = Amount(D("100034.023"), "USD")
        rich_string1 = printer_rich.amount_to_rich_string(amount1)
        expected_rich_string1 = '<span class="amount">100034.023</span> <span class="commodity">USD</span>'
        self.assertEqual(expected_rich_string1, rich_string1)
        # print(rich_string1)
...
if __name__ == "__main__":
    ...
    # testEntryPrinter = TestEntryPrinter()
    # testEntryPrinter.test_Transaction()
    # misc_tests = MiscTests()
    # misc_tests.test_amount_to_rich_string()
    
    # testEntryPrinter = TestEntryPrinter()
    
    # testEntryPrinter.test_Price()
